...
describe('UniswapV2Factory', () => {
  ...
  async function createPair(tokens: [string, string]) {
    ...
    await expect(factory.createPair(...tokens, 0, 0))
      .to.emit(factory, 'PairCreated')
      .withArgs(TEST_ADDRESSES[0], TEST_ADDRESSES[1], create2Address, bigNumberify(1))

    await expect(factory.createPair(...tokens, 0, 0)).to.be.reverted // UniswapV2: PAIR_EXISTS
    await expect(factory.createPair(...tokens.slice().reverse(), 0, 0)).to.be.reverted // UniswapV2: PAIR_EXISTS
  ...
  it('createPair:gas', async () => {
    const tx = await factory.createPair(...TEST_ADDRESSES,0,0)
    const receipt = await tx.wait()
    expect(receipt.gasUsed).to.eq(2707167)
  })
